...
from exprep.load_experiment_logs import read_json
...
def create_page(datasets, dataset_meta):
    ...
    task_configuration = dbc.Offcanvas(
        html.Div(children=[
            html.Div(children=[
                html.H2('Dataset:'),
                dbc.Select(id='ds-switch', value=datasets[0], options=[{'label': dataset_meta[ds]['name'], 'value': ds} for ds in datasets],)
            ]),
            html.Div(children=[
                html.H2('ML Task:'),
